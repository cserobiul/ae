...
{{--                </div>--}}
                <div class="p-5" id="basic-datepicker">
                    <div class="preview">
                         <form action="{{ route('suppliers.update',$supplier->id) }}" method="post" class="validate-form">
                            @csrf
                            @method('PUT')
                            <div>
                                ...
                                <input type="text" name="name" value="{{ old('name',isset($supplier->name)?$supplier->name:null) }}" class="input w-full border mt-2" placeholder="Miraul Hoque" minlength="3" required>
                                @error('name')
                            ...
                            <div class="mt-3">
                                ...
                                <input type="email" name="email" value="{{ old('email',isset($supplier->email)?$supplier->email:null) }}" class="input w-full border mt-2" placeholder="miraz@gmail.com" required>
                                @error('email')
                                <div class="text-danger mb-3">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <label class="flex flex-col sm:flex-row"> Role <span class="sm:ml-auto mt-1 sm:mt-0 text-xs text-gray-600"></span> </label>
                                 <select name="roles[]" data-placeholder="Select Role" class="select2 w-full" multiple>
                                     @foreach($roles as $role)
                                         <option value="{{ $role->name }}" {{ $supplier->hasRole($role->name)? 'selected':'' }}>{{ ucwords($role->name) }}</option>
                                     @endforeach
                                 </select>
                            </div>
